...
grounds. These override any pattern below:

1. **Sentence 1 states the problem — never a finding, never a number.** "It
   starts with a finding rather than the problem" is an instant reviewer flag.
   Why the question matters comes before what was found.
2. **The key finding appears once in plain words before any statistics.** A
   non-specialist should be able to repeat it ("repeat collaboration was rarer
   the farther apart two institutions were"). Only after that plain sentence
   may the estimate appear.
3. **At most three numbers in the whole abstract**: the primary estimate, ONE
   uncertainty statement (the CI *or* the p-value — never both), and at most
   one supporting value. Cohort years and sample sizes are exempt. Everything
   else lives in Results.
4. **No coined analysis labels.** "Field-year cells", "event rows",
   "washout-qualified births", snake_case tokens, column names — say the thing
   in plain words ("the per-year concentration of references within each
   field"); if a technical term is unavoidable, paraphrase it in the same
   sentence.
5. **The second half is not a results table.** main-result → corpus size →
   endpoint → estimand → bootstrap → CI/p → caveat is a compressed table, not
   an abstract. Close with what the finding means, then one scope clause.
...
## English Abstract Quality Checklist

| Check | ✓ |
|-------|---|
| Abstract covers all 5 components | |
| 150-300 words unless venue policy requires otherwise | |
| Claims match the verified manuscript | |
| Keywords: 5-7 | |
