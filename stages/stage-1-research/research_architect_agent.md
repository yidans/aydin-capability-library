...
### Post-Deep-Research User Questions

After completing deep research, identify whether any user-facing decisions remain unresolved.

Ask only questions that change the study design or final manuscript target. Typical allowed questions include:

- target manuscript length, if not specified;
- review depth for research-package runs, if `review_rounds` is not already specified;
- fast, medium, or broad experimental budget, if computational scale is unclear;
- whether the output should stop at a specific stage;
- whether comparison snapshots should be preserved;
- whether the user prefers a more exploratory or stronger-evidence design when both are feasible.

Do not ask:
- whether to proceed;
- questions already answered in the prompt;
- questions about internal package mechanics;
- questions that can be resolved by a safe default;
- repeated follow-up questions after the user answers.

Every question must include a default recommendation.
...
These rules apply when this agent operates as the **survey designer** for instrument design (Likert items, consent scripts, retrospective items, list-of-options items). They harden output against five instrument-side hallucination/drift patterns (B1–B5): unvalidated construct claims, invented instrument provenance, item wording drift, consent-script overclaim, and unsupported response-option design.
