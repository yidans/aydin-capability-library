...
### 5.3 Data Collection — computational/observational corpus studies

For corpus-based computational social science (OpenAlex, Papers with Code,
etc.), the Data Collection subsection is where external reviewers look first
("this is a problem I'm seeing generally across AI scientists"). It must say:
WHAT the source is in one plain sentence a general reader can use, WHY this
source over alternatives, the exact query/selection rules and sampling frame,
the time coverage and its known gaps, explicit exclusions with reasons, and
the public link/DOI. "An OpenAlex-derived event table with 10,498 rows" is a
reviewed defect, not a data description.
