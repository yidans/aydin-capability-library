...
A recent automated-researcher system (2026) uses a three-tier sandbox: local
...
ARS pipelines enforce the spirit of this isolation: research questions are set
outside benchmark internals, outputs are checked at integrity gates, and
calibration gold sets are supplied at runtime rather than embedded in the
repository. This document makes the pattern explicit and machine-checkable via
the `data_access_level` annotation declared in internal `CAPABILITY.md` / `PIPELINE.md`
contracts, plus the root public skill routing contract.
...
flow is rigorously one-way. An artifact can be promoted from a lower layer to a
...
The `data_access_level` annotation in capability or pipeline frontmatter maps
...
|---|---|
| Source verification (S2 API) | `capabilities/deep-research/references/semantic_scholar_api_protocol.md` |
| Anti-leakage protocol | `capabilities/academic-paper/references/anti_leakage_protocol.md` |
...
This pattern document is the narrative rationale; those five reference files
...
Version 3.3.2 shipped the pattern document, the `data_access_level` annotation
across the original skill files, and the `task_type` annotation. The autonomous
single-entry package keeps those annotations on internal capability contracts.
...
(directly analogous to a server-side evaluation tier); per-agent
